...
/**
 * Retrieves one or more `BankEmployeeShape` records from the in-memory dataset.
 *
 * This helper supports three usage modes:
 *
 * 1. **`getBankEmployeeData()`**
 *    Returns a deep-cloned copy of the entire employee dataset.
 *
 * 2. **`getBankEmployeeData(index)`**
 *    Returns a cloned copy of a single employee at the given index.
 *
 * 3. **`getBankEmployeeData(index, true)`**
 *    Returns the selected employee wrapped in a one-element array, which can be
 *    useful when a FeatureCell or pipeline expects an array-like payload.
 *
 * The function always returns *cloned* data via `isolateValue`, ensuring the
 * shared dataset cannot be mutated by callers.
 *
 * @param index - Optional dataset index to retrieve. If omitted or invalid,
 *                the full dataset is returned.
 * @param asArray - When `true`, wraps a single returned item in an array.
 * @returns A cloned `BankEmployeeShape` record, an array containing one item,
 *          or a cloned array of all employee records.
 */
export function getBankEmployeeData(
  index?: number,
  asArray = false
