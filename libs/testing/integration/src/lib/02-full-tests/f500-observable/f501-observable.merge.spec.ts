...
import { f501Snapshot } from './snap-shots/f501-observable.merge.snapshot';
 ...
 *
 *
 * merge is testService.vault.replaceState(of(getBankEmployeeData() as BankEmployeeShape[]));
...
describe('f501: Value - Merge with Encrypt Test', () => {
  const key = 'full-test';
  ...
  it('should have the correct insight events', async () => {
    jasmine.clock().tick(1000);
    await vaultSettled(key);
    expectMonitorSnapshot(emitted, f501Snapshot);
  });
});
