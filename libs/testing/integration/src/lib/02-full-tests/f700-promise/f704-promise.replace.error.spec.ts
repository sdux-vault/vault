...
import { f704Snapshot } from './snap-shots/f704-promise.replace.error.snapshot';

describe('f704: Promise - Replace - With encryption - Error Test', () => {
  const key = 'full-test';
  ...
  it('should errors on a replace through the entire pipe', async () => {
    ...
    testService.vault.replaceState(
      testService.formatPromiseInputAsValueReject('The reject error')
    );
    await vaultSettled(key);
  ...
  it('should have the correct insight events', async () => {
    ...
    expectMonitorSnapshot(emitted, f704Snapshot);
  });
});
