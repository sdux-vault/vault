...
import { f1001Snapshot } from './snap-shots/f1001-from-deferred.merge.snapshot';
...
describe('f1001: Value - fromDeferred Merge with Encrypt Test', () => {
  const key = 'full-test';
  ...
  it('should merge through the entire pipe', async () => {
    ...
    expect(getLocalStorage(storageKey)).not.toBeNull();
  ...
  it('should have the correct insight events', async () => {
    jasmine.clock().tick(1_000);
    await vaultSettled(key);
    expectMonitorSnapshot(emitted, f1001Snapshot);
  });
});
