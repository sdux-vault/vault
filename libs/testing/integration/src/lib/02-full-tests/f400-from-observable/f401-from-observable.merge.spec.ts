...
import { f401Snapshot } from './snap-shots/f401-from-observable.merge.snapshot';
...
describe('f401: Value - FromObservable Merge with Encrypt Test', () => {
  const key = 'full-test';
  ...
  it('should merge through the entire pipe', async () => {
    ...
    testService.vault.fromObservable!(
      of(getBankEmployeeData(6, true) as any)
    ).subscribe({
    ...
    testService.vault.fromObservable!(
      of(getBankEmployeeData(7, true) as any)
    ).subscribe({
      next: (vaultInput: VaultStateRef<BankEmployeeShape[]>): void => {
        testService.vault.replaceState(vaultInput);
      }
    ...
    testService.vault.fromObservable!(of([] as any)).subscribe({
      next: (vaultInput: VaultStateRef<BankEmployeeShape[]>): void => {
        testService.vault.mergeState(vaultInput);
      }
    });
  ...
  it('should have the correct insight events', async () => {
    jasmine.clock().tick(1_000);
    await vaultSettled(key);
    expectMonitorSnapshot(emitted, f401Snapshot);
  });
});
