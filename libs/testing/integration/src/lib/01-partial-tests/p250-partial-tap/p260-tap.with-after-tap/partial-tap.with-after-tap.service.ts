 ...
 * after reducers and filters.
 *
 * This class is part of the **p260 Tap Pipeline Integration Suite** and
 * specifically verifies after-tap behavior (as opposed to after-tap).
 */
@FeatureCell<BankEmployeeShape[]>('partial-with-after-tap')
@Injectable({
  providedIn: 'root'
})
export class PartialWithAfterTapService extends PartialTapAbstractClass<
  BankEmployeeShape[]
> {
  /**
   * Constructs a FeatureCell configured with a full set of after-tap
   * test functions. The base class receives `'After'` as its identifier,
   * allowing each tap to brand its source for later inspection.
   */
  constructor() {
    super(
      injectVault<BankEmployeeShape[]>(PartialWithAfterTapService),
      'After'
    );
  }
    ...
    this.vault
      .afterTaps([
        ...
        (tap): void => {
          const extendedTap = {
            value: tap
          } as ExtendedStateSnapshot<BankEmployeeShape[]>;
  ...
  initializeError(): void {
    this.vault
      .afterTaps([
        ...
        // Inline tap used to simulate a runtime failure inside afterTaps
        (value): void => {
