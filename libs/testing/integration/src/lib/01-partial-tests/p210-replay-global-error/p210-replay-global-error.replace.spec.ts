...
import { p210Snapshot } from './snap-shots/p210-replay-global-error.replace.snapshot';

describe('p210: Replay Global Error - Replace Test', () => {
  ...
  it('should replace the bank employees with global pause and replace', async () => {
    const state = testService.getState();

    await testService.vault.replaceState(
      Object({ value: getBankEmployeeData() })
    );
    ...
    await testService.vault.replaceState(getBankEmployeeData(0, true) as any);
    await flushVaultPipeline();
  ...
  it('should have the correct insight events', async () => {
    await flushVaultPipeline();
    expectMonitorSnapshot(emitted, p210Snapshot);
  });
});
