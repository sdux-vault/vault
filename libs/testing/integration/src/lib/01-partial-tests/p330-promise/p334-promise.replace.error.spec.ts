...
import { p334Snapshot } from './snap-shots/p334-promise.replace.error.snapshot';
...
describe('f334: From Promise - Replace Error Test', () => {
  ...
  it('should replace after receiving a fromPromise', async () => {
    ...
    testService.vault.replaceState(
      testService.formatPromiseInputAsValue(getBankEmployeeData(1, true))
    );
    await vaultSettled(key);
  ...
  it('should have the correct insight events', async () => {
    ...
    expectMonitorSnapshot(emitted, p334Snapshot);
  });
});
