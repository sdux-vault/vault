...
import { p055Snapshot } from './snap-shots/p055-from-stream.push-merge.no-auto-reset.error.snapshot';
...
describe('p055: From Stream - with filter no auto reset error Merge', () => {
  const key = 'partial-from-stream';
  ...
  it('should merge through the entire pipe', async () => {
    testService.vault.fromStream!(employees$, { autoResetError: false });
    ...
    expect(state.error()).toEqual(
      Object({
        ...
        featureCellKey: 'partial-from-stream'
      })
    );
    expect(state.isLoading()).toBeFalse();
  ...
  it('should have the correct insight events', async () => {
    ...
    expectMonitorSnapshot(emitted, p055Snapshot);
  });
});
