...
import { p042Snapshot } from './snap-shots/p042-from-observable.replace.snapshot';
...
describe('p042: From Observable - Replace Reset', () => {
  ...
  it('should replace after receiving a fromObservable', async () => {
    ...
    testService.vault.fromObservable!(
      testService.getSource(null as any) as any
    ).subscribe({
      next: (vaultInput: VaultStateRef<BankEmployeeShape[]>): void => {
        testService.vault.replaceState(vaultInput);
      }
  ...
  it('should have the correct insight events', async () => {
    await flushVaultPipeline();
    expectMonitorSnapshot(emitted, p042Snapshot);
  });
});
