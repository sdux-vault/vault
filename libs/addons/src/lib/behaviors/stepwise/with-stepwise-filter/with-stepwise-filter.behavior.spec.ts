...
import { withStepwiseFilterBehavior } from './with-stepwise-filter.behavior';

describe('Behavior: Stepwise Filter', () => {
  ...
  describe('withStepwise with config', () => {
    beforeEach(() => {
      ...
      behavior = new withStepwiseFilterBehavior('stepwise-behavior', {
        behaviorConfig: {
          stepwiseCallback: (
            behaviorCurrent: any,
    ...
    it('should have correct metadata', () => {
      expect(behavior.type).toBe('stepwiseFilter');
      expect(behavior.critical).toBeTrue();
      expect(behavior.key).toBe('stepwise-behavior');
    });

    it('should have default decorator properties', () => {
      expect(withStepwiseFilterBehavior.critical).toBeTrue();
      expect(withStepwiseFilterBehavior.type).toBe('stepwiseFilter');
      expect(withStepwiseFilterBehavior.key).toBe(
        'SDUX::Behavior::Policy::StepwiseFilter'
      );
      expect(withStepwiseFilterBehavior.wantsConfig).toBeTrue();
      expect(withStepwiseFilterBehavior.configKey).toBe('withStepwiseFilter');
      expect(typeof withStepwiseFilterBehavior.installFluentApi).toBe(
        'function'
      );
    });
    ...
    it('should emit a StepwiseRequest with correct shape for continue', async () => {
      ...
      callbacks.continue();
      expect(callbacks.stage).toBe('filter');
      ...
      answerSubject.next({
        id: `${pipelineId}:Filter:TestStepwise`,
        decision: 'continue'
      });
      ...
      expect(emitted).toEqual([
        Object({
          id: 'p1:Filter:stepwise-behavior',
          pipelineId: 'p1',
          stage: 'filter',
          snapshot
        }),
    ...
    describe('fluent api: installFluentApi()', () => {
      ...
      beforeEach(() => {
        ...
        // install fluent api
        withStepwiseFilterBehavior.installFluentApi(cell, behaviorConfigs);
      });
      ...
      it('should store options in behaviorConfigs under withStepwiseFilter key', () => {
        const options = {
          idKey: 'id',
          fetch: jasmine.createSpy('fetch')
        };

        cell.withStepwiseFilter(options);

        expect(behaviorConfigs.has('withStepwiseFilter')).toBeTrue();
        expect(behaviorConfigs.get('withStepwiseFilter')).toBe(options);
      });
      ...
      it('should return the cell to allow fluent chaining', () => {
        ...
        const result = cell.withStepwiseFilter(options);
      ...
      it('should overwrite previous cache config when called again', () => {
        ...
        cell.withStepwiseFilter(options1);
        cell.withStepwiseFilter(options2);

        expect(behaviorConfigs.get('withStepwiseFilter')).toBe(options2);
      });

      it('should propagate errors thrown by stepwiseCallback', async () => {
        const behavior = new withStepwiseFilterBehavior('stepwise-behavior', {
          behaviorConfig: {
            stepwiseCallback: () => {
  ...
  describe('without config', () => {
    it('should throw if options are missing', () => {
      expect(
        () =>
          new withStepwiseFilterBehavior<any>(
            'behavior-key',
            {} as BehaviorClassContext
          )
      ).toThrowError(
        '[vault] Stepwise behavior requires configuration via withStepwiseFilter()'
      );
    });
    ...
    it('should throw if fetch option is not a function', () => {
      expect(
        () =>
          new withStepwiseFilterBehavior<any>('behavior-key', {
            behaviorConfig: {}
          } as BehaviorClassContext)
      ).toThrowError(
