...
import { withStepwiseReducerBehavior } from './with-stepwise-reducer.behavior';

describe('Behavior: Stepwise Reducer', () => {
  ...
  describe('withStepwise with config', () => {
    beforeEach(() => {
      ...
      behavior = new withStepwiseReducerBehavior('stepwise-behavior', {
        behaviorConfig: {
          stepwiseCallback: (
            behaviorCurrent: any,
    ...
    it('should have correct metadata', () => {
      expect(behavior.type).toBe('stepwiseReducer');
      expect(behavior.critical).toBeTrue();
      expect(behavior.key).toBe('stepwise-behavior');
    });

    it('should have default decorator properties', () => {
      expect(withStepwiseReducerBehavior.critical).toBeTrue();
      expect(withStepwiseReducerBehavior.type).toBe('stepwiseReducer');
      expect(withStepwiseReducerBehavior.key).toBe(
        'SDUX::Behavior::Policy::StepwiseReducer'
      );
      expect(withStepwiseReducerBehavior.wantsConfig).toBeTrue();
      expect(withStepwiseReducerBehavior.configKey).toBe('withStepwiseReducer');
      expect(typeof withStepwiseReducerBehavior.installFluentApi).toBe(
        'function'
      );
    });
    ...
    it('should emit a StepwiseRequest with correct shape for continue', async () => {
      ...
      callbacks.continue();
      expect(callbacks.stage).toBe('reducer');
      ...
      answerSubject.next({
        id: `${pipelineId}:Reducer:TestStepwise`,
        decision: 'continue'
      });
      ...
      expect(emitted).toEqual([
        Object({
          id: 'p1:Reducer:stepwise-behavior',
          pipelineId: 'p1',
          stage: 'reducer',
          snapshot
        }),
    ...
    describe('fluent api: installFluentApi()', () => {
      ...
      beforeEach(() => {
        ...
        // install fluent api
        withStepwiseReducerBehavior.installFluentApi(cell, behaviorConfigs);
      });
      ...
      it('should store options in behaviorConfigs under withStepwiseReducer key', () => {
        const options = {
          idKey: 'id',
          fetch: jasmine.createSpy('fetch')
        };

        cell.withStepwiseReducer(options);

        expect(behaviorConfigs.has('withStepwiseReducer')).toBeTrue();
        expect(behaviorConfigs.get('withStepwiseReducer')).toBe(options);
      });
      ...
      it('should return the cell to allow fluent chaining', () => {
        ...
        const result = cell.withStepwiseReducer(options);
      ...
      it('should overwrite previous cache config when called again', () => {
        ...
        cell.withStepwiseReducer(options1);
        cell.withStepwiseReducer(options2);

        expect(behaviorConfigs.get('withStepwiseReducer')).toBe(options2);
      });

      it('should propagate errors thrown by stepwiseCallback', async () => {
        const behavior = new withStepwiseReducerBehavior('stepwise-behavior', {
          behaviorConfig: {
            stepwiseCallback: () => {
  ...
  describe('without config', () => {
    it('should throw if options are missing', () => {
      expect(
        () =>
          new withStepwiseReducerBehavior<any>(
            'behavior-key',
            {} as BehaviorClassContext
          )
      ).toThrowError(
        '[vault] Stepwise behavior requires configuration via withStepwiseReducer()'
      );
    });
    ...
    it('should throw if fetch option is not a function', () => {
      expect(
        () =>
          new withStepwiseReducerBehavior<any>('behavior-key', {
            behaviorConfig: {}
          } as BehaviorClassContext)
      ).toThrowError(
