...
describe('CLI: SduxComparisonSourceGenerator', () => {
  ...
  it('should calculate comparison line metadata', () => {
    const metadata = createGenerator().createOutputMetadata(
      [{ source: 'competitor\nsource\nsource' }],
      [{ source: 'sdux\nsource' }]
    );

    expect(metadata).toEqual({
      competitorTotalLines: 3,
      sduxTotalLines: 2,
      lineDifference: 1,
      percentageSavings: 33.33
    });
  });
  ...
  it('should serialize comparison files into an exported TypeScript constant', () => {
    ...
    expect(generated).toContain('export const ANGULAR_SDUX_OUTPUT_METADATA =');
