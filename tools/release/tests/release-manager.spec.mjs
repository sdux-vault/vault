...
describe('CLI: release-manager', () => {
  ...
  it('runs full release pipeline with normalized paths', () => {
    ...
    expect(execCalls).toEqual([
      ...
      'git commit -m "chore(shared): 1.2.4 release version bump"',
  ...
  it('skips execution in dry run', () => {
    ...
    expect(consoleInfo).toEqual([
      ...
      'Building build:shared build:devtools build:core build:addons build:angular build:react build:svelte build:vue...',
      'npm run build:shared',
      'npm run build:devtools',
      'npm run build:core',
      'npm run build:addons',
