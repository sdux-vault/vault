...
export const arrayAppendMergeExampleProject: Project = {
  title: 'angular-array-append-merge-example',
  template: 'node',
  files: {
    'angular.json': `{
...
`,
    'package.json': `{
  "name": "angular-array-append-merge-example",
  "version": "1.0.3",
  "private": true,
  "scripts": {
...
`,
  ...
  <div class="header">
    <div class="title">Angular - SDuX Vault Array Append Merge Example</div>
    <div class="subtitle">
...
export class ExampleService {
   ...
   * \`initialize()\` processes that value through the full pipeline to seed state
   * before any consumer reads it. After initialization, all \`mergeState()\` calls
   * pass through the \`withArrayAppendMergeBehavior\`, which concatenates the
   * incoming array with the existing state array.
   */
  constructor() {
   ...
   *
   * The incoming array is not written directly — it passes through the merge
   * stage of the pipeline, where the behavior concatenates it with the current
   * state, producing a new combined array as the final state value.
   *
   ...
   * To return to a specific value, call \`merge()\` with the desired data.
   *
   * @returns void
   */
  reset(): void {
