...
export const stateIntrospectionTutorialExampleProject: Project = {
  title: 'state-introspection-tutorial-example',
  template: 'node',
  files: {
    'angular.json': `{
...
`,
    'package.json': `{
  "name": "state-introspection-tutorial-example",
  "version": "1.0.0",
  "private": true,
  "scripts": {
  ...
  <fieldset class="feature-cell-controls">
      ...
      <div class="operator-actions">
        <div class="action-row lifecycle-action-row">
          <input
            ...
            type="checkbox"
            aria-label="Show Restore Initial Data description" />
          <div class="button-container">
            <!-- Teaching point: Restore (ex-022) -->
            <button
              type="button"
              class="button warn"
              (click)="restoreInitialCharacters()">
              Restore Initial Data
            </button>
            <label
              class="description-chevron"
              for="restore-description"
              title="Show or hide Restore Initial Data description"></label>
          </div>
          <div class="description-container">
            Replaces the current collection with its captured initial values and
            selects the first restored character.
          </div>
        </div>
    ...
    <div class="state-output">
      ...
      <div class="section-header">
        ...
        <label
          class="section-chevron"
          for="state-section-toggle"
          title="Show or hide State"></label>
      </div>

      <div class="state-content">
        <input
          id="raw-state-output-toggle"
          class="tap-toggle"
          type="checkbox"
          aria-label="Toggle State output visibility"
          checked />
        ...
        <div class="tap-column">
          <div class="tap-header">
            ...
            <label
              class="tap-chevron"
              for="raw-state-output-toggle"
              title="Show or hide Initial State output"></label>
          </div>
          <textarea
            readonly
            rows="8"
            aria-label="Initial State output"
            [value]="originalStateJson"></textarea>
        </div>

        <div class="tap-column">
          <div class="tap-header">
            <h3>Raw StateSnapshot</h3>
            <label
              class="tap-chevron"
              for="raw-state-output-toggle"
              title="Show or hide Raw StateSnapshot output"></label>
          </div>
          <textarea
            readonly
            rows="8"
            aria-label="Raw StateSnapshot output"
            [value]="rawStateJson()"></textarea>
        </div>
        ...
        <div class="tap-column">
          ...
          <textarea
            ...
            rows="8"
            aria-label="Raw StateSnapshot\$ output"
            [value]="rawStateStreamJson()"></textarea>
        </div>
      </div>
        ...
        <div class="tap-content">
          ...
          <div class="tap-column">
            ...
            <textarea
              readonly
              rows="8"
              aria-label="Before Taps output"
              [value]="beforeTapInputJson()"></textarea>
          </div>
          ...
          <div class="tap-column">
            ...
            <textarea
              readonly
              rows="8"
              aria-label="After Taps output"
              [value]="afterTapInputJson()"></textarea>
          </div>

          <div class="tap-column">
            <div class="tap-header">
              <h3>State Emission</h3>
              <label
                class="tap-chevron"
                for="tap-output-toggle"
                title="Show or hide pipeline output"></label>
            ...
            <textarea
              readonly
              rows="8"
              aria-label="State Emission output"
              [value]="emittedStateJson()"></textarea>
          </div>
        </div>
      </div>
    </div>
...
describe('ExampleComponent', () => {
  ...
  it('should expose the raw state, stream, tap, and emission teaching outputs', async () => {
    await vaultSettled(key);
    fixture.detectChanges();

    const expectedSnapshot = {
      isLoading: false,
      value: reducedCharacters,
      error: null,
      hasValue: true
    };

    expect(component['rawStateJson']()).toBe(
      JSON.stringify(expectedSnapshot, null, 2)
    );
    expect(component['rawStateStreamJson']()).toBe(
      JSON.stringify(expectedSnapshot, null, 2)
    );
    expect(component['beforeTapInputJson']()).toBe(
      JSON.stringify(initialCharacters, null, 2)
    );
    expect(component['afterTapInputJson']()).toBe(
      JSON.stringify(reducedCharacters, null, 2)
    );
    expect(component['emittedStateJson']()).toBe(
      JSON.stringify(expectedSnapshot, null, 2)
    );
  });

  it('should restore the captured initial collection and selected character', async () => {
    await vaultSettled(key);
    fixture.detectChanges();
    ...
    expect(component['feedback']()).toEqual(
      component.editor.feedback['initialCollectionRestored']
    );
  });

  it('should restore an empty baseline in create mode', async () => {
    await vaultSettled(key);
    const restoreInitialCharactersSpy = spyOn(
      service,
    ...
    expect(restoreInitialCharactersSpy).toHaveBeenCalledOnceWith();
    expect(component['selectedCharacterId']()).toBeNull();
  ...
  it('should reset FeatureCell state and clear the editor form', async () => {
    await vaultSettled(key);
    fixture.detectChanges();
    const resetStateSpy = spyOn(service, 'resetState').and.callThrough();
    ...
    expect(resetStateSpy).toHaveBeenCalledOnceWith();
...
export class ExampleService {
  ...
  constructor() {
    /**
     * Captures the initial set of characters from the committed state and prepares them for the tutorial.
     */
