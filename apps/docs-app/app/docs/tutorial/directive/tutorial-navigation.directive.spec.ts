...
@Component({
  ...
  template: `
    <nav>
      <a
        [class.tutorial-step-active]="isActiveStep('chapter-1-step-1')"
        [attr.aria-current]="isAriaStep('chapter-1-step-1')"
        >Step One</a
      >
      <a
        [class.tutorial-step-active]="isActiveStep('chapter-1-step-2')"
        [attr.aria-current]="isAriaStep('chapter-1-step-2')"
        >Step Two</a
      >
    </nav>
    <main class="tutorial-content">
      <section class="section" id="chapter-1-step-1"></section>
      <section class="section" id="chapter-1-step-2"></section>
      <section class="section" id="chapter-1-step-3"></section>
    </main>
  `
})
...
describe('Directive: TutorialNavigation', () => {
  ...
  it('should observe every tutorial section and default to step one', async () => {
    ...
    expect(component.activeStep()).toBe('chapter-1-step-1');
  ...
  it('should select the intersecting section nearest the top', async () => {
    ...
    const observer = IntersectionObserverMock.instances[0];
    const stepOne = fixture.nativeElement.querySelector('#chapter-1-step-1');
    const stepTwo = fixture.nativeElement.querySelector('#chapter-1-step-2');
    const stepThree = fixture.nativeElement.querySelector('#chapter-1-step-3');
    ...
    expect(component.activeStep()).toBe('chapter-1-step-2');
  ...
  it('should report the active and aria state for a step', async () => {
    ...
    component.activeStep.set('chapter-1-step-3');

    expect(component.isActiveStep('chapter-1-step-3')).toBeTrue();
    expect(component.isActiveStep('chapter-1-step-2')).toBeFalse();
    expect(component.isAriaStep('chapter-1-step-3')).toBe('step');
    expect(component.isAriaStep('chapter-1-step-2')).toBeNull();
  });
  ...
  it('should scroll the fragment target into view', async () => {
    ...
    fragment$.next('chapter-1-step-2');
